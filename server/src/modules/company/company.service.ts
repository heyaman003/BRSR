...
export class CompanyService {
  ...
  async getCompanyDetails(companyId: string): Promise<CompanyDto> {
    const company: Company | null =
      await this.companyRepository.getCompanyDetails(companyId);
  ...
  async listAllCompanies(): Promise<any[]> {
    const allCompanies: Company[] =
      await this.companyRepository.listCompanies();
    ...
    return {
      ...
      users: company['users']
        ? company['users'].map((user: User) =>
            this.userService.convertToDto(user),
          )
        : [],
