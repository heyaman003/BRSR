...
export class UserController {
  ...
  async createAdmin(
    @Body(ValidationPipe) newAdmin: CreateUserDto,
  ): Promise<ResponseModel> {
  ...
  @Role(UserRole.ADMIN)
  async createClient(
    @Body(ValidationPipe) newClient: CreateUserDto,
  ): Promise<ResponseModel> {
    const newUser = await this.userService.createUser(
      newClient,
      UserRole.CLIENT,
  ...
  async getUserWithAssignedQuestions(
    @Req() request: Request,
  ): Promise<ResponseModel> {
    Logger.log(request['user']);
    const userId: string = request['user']['sub'];
    const getUserWithAssignedQuestions =
      await this.userService.getUserWithAssignedQuestions(userId);
    console.log(getUserWithAssignedQuestions, 'getUserWithAssignedQuestions');
    return new ResponseModel(
      HttpStatus.OK,
      'Success',
      getUserWithAssignedQuestions,
    );
  }
}
