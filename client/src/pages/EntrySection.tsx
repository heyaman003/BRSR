...
export default function QuestionnairePage() {
  const [activeSection, setActiveSection] =
    useState<keyof typeof sectionsd>("A");
  const [activeSectionTab, setActiveSectionTab] = useState("Collectiveinput");
  const activeSectionData = sections[activeSection].find(
    (s) => "id" in s && s.id === activeSectionTab
  );
      ...
      <div className=" mx-auto ">
        <div className="grid grid-cols-1 md:grid-cols-[350px_1fr] gap-8 h-[100vh] overflow-y-auto bg-[#f2f9fa]">
          ...
          <div className=" rounded-lg p-6 pt-0 h-[100vh] overflow-y-auto ">
              ...
              <>
                ...
                activeSectionData.type === "normal" ? (
                      ...
                      question?.type === "confirm" ? (
                        <div key={question?.id} className="space-y-2">
                          ...
                          <div className={cn("flex gap-6")}>
                            <button
                              className={cn(
                                "py-1 px-8 text-white bg-green-500 rounded-sm text-sm"
                              )}
                            >
                              Yes
                            </button>
                            <button
                              className={cn(
                                "py-1 px-8 text-white bg-red-500 rounded-sm text-sm"
                              )}
                            >
                              No
                            </button>
                  ...
                  <>
                    ...
                    {activeSectionData?.tabletype === 4 && (
                      <TableHelper3 section={activeSectionData} />
                    )}
                  </>
                ) : null}
              </>
            ...
            <Button
              type="submit"
              className="w-[400px]  mt-5 bg-yellow-500 hover:bg-yellow-600 text-white"
            >
              {" "}
              Save and Next{" "}
            </Button>
