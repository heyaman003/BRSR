...
const CompanyUser = () => {
  ...
  const userRole = useSelector((state: RootState)=>state.auth.user?.data.role)
  ...
  const deleteUserFromState = (userId: string) => {
    setCompanyData((companyData) => {
      ...
      companyData.users = companyData.users.filter(
  ...
  return (
    <div className="bg-green-100 min-h-screen bg-gradient-to-b from-background to-accent/50 pb-16 overflow-hidden relative">
      ...
      <div className="container max-w-5xl mx-auto relative z-10">
        ...
        {/* Button that naviagates to registerd company listing page */}
        {userRole ==='SUPERADMIN' && <Button
          className="rounded-full size-10 mt-2"
          onClick={() => navigate("/brsr-making")}
        >
          <ArrowLeft />
        </Button>}
        <Button
          onClick={activeCompanyDetails}
          className="absolute right-0 w-[200px] top-[10px] capitalize bg-green-500 hover:bg-green-600"
        >
          view company data
        </Button>
